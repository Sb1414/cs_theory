...
+ [В чем разница между public, static и void ?](#в-чем-разница-между-public-static-и-void-)
...
Главная мотивация:
1. Совместимость с windows API, хранение "честного" указателя на функцию 
...
```
- as - закидывает в переменную (var) либо результат, если получилось преобразовать, либо null - если не получилось

- is - возвращает либо true, либо false

С помощью оператора **as** программа пытается преобразовать выражение к определенному типу (работает только на значимых типах), при этом не выбрасывает исключение. В случае неудачного преобразования выражение будет содержать значение null.
...
## Что такое C# ?
C# — это объектно-ориентированный язык программирования общего назначения, разработанный Microsoft. Его часто используют для разработки различных типов приложений, включая веб-, десктопные и мобильные приложения. 

## В чем разница между public, static и void ?
**Публично** объявленные переменные или методы доступны в любом месте приложения. Статические объявленные переменные или методы доступны глобально без создания экземпляра класса. 

**Статический** член по умолчанию не доступен глобально, это зависит от используемого типа измененного доступа. Компилятор сохраняет адрес метода в качестве точки входа и использует эту информацию, чтобы начать выполнение до создания каких-либо объектов. 

**Void** — это модификатор типа, который указывает, что метод или переменная не возвращает никакого значения.
